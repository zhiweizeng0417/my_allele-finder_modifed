...
from collections import defaultdict  # Moved to top
...
class AlleleUtils:
    ...
    def adjust_allele_table(in_allele, ref_gff3, hap_gff3, ref_blast, hap_blast, iden, tandem, allele_num, out_allele,
        ...
        tandem_set = set() # Initialize here, as it's modified by the following logic

        direct_tandem_relationships = defaultdict(list)
        genes_as_first_in_pair = set()
        genes_as_second_in_pair = set()

        try:
            with open(tandem, 'r') as fin: # 'tandem' is the function parameter here
                for line in fin:
                    gene_ids_in_pair = line.strip().split(',')

                    if len(gene_ids_in_pair) != 2:
                        print(f"Warning: Skipping line with unexpected number of genes in tandem file: '{line.strip()}'", file=sys.stderr)
                        continue

                    gene1_id = gene_ids_in_pair[0]
                    gene2_id = gene_ids_in_pair[1]

                    if gene1_id not in hap_db or gene2_id not in hap_db:
                        print(f"Warning: Gene(s) '{gene1_id}' or '{gene2_id}' not found in hap_db, skipping pair: '{line.strip()}'", file=sys.stderr)
                        continue

                    chr1 = hap_db[gene1_id][0]
                    chr2 = hap_db[gene2_id][0]

                    if chr1 == chr2:
                        direct_tandem_relationships[gene1_id].append(gene2_id)
                        genes_as_first_in_pair.add(gene1_id)
                        genes_as_second_in_pair.add(gene2_id)

        except FileNotFoundError:
            print(f"Error: Tandem file not found at '{tandem}'. Please ensure the path is correct.", file=sys.stderr)
            sys.exit(1)
        except Exception as e:
            print(f"An unexpected error occurred while processing tandem file '{tandem}': {e}", file=sys.stderr)
            sys.exit(1)

        chain_start_genes = genes_as_first_in_pair - genes_as_second_in_pair

        for start_gene in chain_start_genes:
            current_chain = []
            queue = [start_gene]
            visited = set()
            ...
            while queue:
                ...
                for next_gene in direct_tandem_relationships[gene]:
                    if next_gene not in visited:
                        queue.append(next_gene)

            if len(current_chain) > 1:
                for gene_in_chain in current_chain[1:]:
                    tandem_set.add(gene_in_chain)

        # The rest of the function continues here, now that tandem_set is correctly populated
